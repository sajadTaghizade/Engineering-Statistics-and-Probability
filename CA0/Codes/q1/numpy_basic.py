...
def mutate_array(x, indices, values):
    ...
    - The input array x
    """
    # Mutate the array based on the indices and values
...
def multiples_of_ten(start, stop):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x=np.arange(10*(int(start/10)+1),10*(int(stop/10)+1),10, dtype=float)
...
def slice_indexing_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  out=[(x[2,:]),(x[:,2:3]),(x[0:2,0:3]),(x[0::2,1::2])]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return out
...
def reshape_practice(x):
    ...
    """
    # Reshape x to a 2D array with 3 rows and 8 columns
    y = x.reshape(3, 8)

    # Rearranging the values to fit the specified structure
    y = np.array([
        [y[0, 0], y[0, 1], y[0, 2], y[0, 3], y[1, 4], y[1, 5], y[1, 6], y[1, 7]],
        [y[0, 4], y[0, 5], y[0, 6], y[0, 7], y[2, 0], y[2, 1], y[2, 2], y[2, 3]],
        [y[1, 0], y[1, 1], y[1, 2], y[1, 3], y[2, 4], y[2, 5], y[2, 6], y[2, 7]]
    ])
...
def zero_row_min(x):
  ...
  # Replace "pass" statement with your code
  y = x.copy()
  min_ind = np.argmin(x, axis=1)
  y[np.arange(x.shape[0]), min_ind] = 0
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
        ...
        B, N, M = x.shape
        _, _, P = y.shape
        z = np.empty((B, N, P), dtype=x.dtype)
        for i in range(B):
...
def normalize_columns(x):
  ...
  M = x.shape[0]  
  column_means = np.sum(x, axis=0) / M  
    
  squared_diff = (x - column_means) ** 2
    
  column_variances = np.sum(squared_diff, axis=0) / M
    
  column_stds = np.sqrt(column_variances)
    
   
  y = (x - column_means) / column_stds
